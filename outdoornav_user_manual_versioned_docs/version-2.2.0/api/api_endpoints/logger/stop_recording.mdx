...
int main(int argc, char **argv)
{
  ...
  std::shared_ptr<rclcpp::Node> node = rclcpp::Node::make_shared("stop_logger_client");
  rclcpp::Client<std_srvs::srv::Trigger>::SharedPtr client =
    node->create_client<std_srvs::srv::Trigger>("/" + namespace_ + "/" + std::string("log_manager/stop_recording"));

  auto request = std::make_shared<std_srvs::srv::Trigger::Request>();

  while (!client->wait_for_service(2s)) {
    if (!rclcpp::ok()) {
      RCLCPP_ERROR(rclcpp::get_logger("stop_logger_client"), "Interrupted while waiting for the service. Exiting.");
      return 0;
    }
    RCLCPP_INFO(rclcpp::get_logger("stop_logger_client"), "/%s/log_manager/stop_recording service not available, waiting again...", namespace_.c_str());
  }
    ...
    rclcpp::FutureReturnCode::SUCCESS)
  {
    ...
    {
      RCLCPP_INFO(rclcpp::get_logger("stop_logger_client"), "Logger stopped!");
    }
    else
    {
      RCLCPP_ERROR(rclcpp::get_logger("stop_logger_client"), "Failed to stop logger!");
    }
  } else {
    RCLCPP_ERROR(rclcpp::get_logger("stop_logger_client"), "Failed to call service /%s/log_manager/stop_recording", namespace_.c_str());
  }
